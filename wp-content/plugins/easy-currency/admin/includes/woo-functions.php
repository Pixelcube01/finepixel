...
class ECCW_WOO_FUNCTIONS extends ECCW_Plugin_Settings { 
    ...
    public function __construct(){
        ...
        if (!is_admin()) {
            ...
            add_filter( 'woocommerce_package_rates', [ $this, 'eccw_convert_shipping_costs' ], 9999, 2 );
            add_action( 'woocommerce_before_calculate_totals', [ $this, 'eccw_clear_cached_shipping_rates' ] );
    ...
    /**
     * Convert WooCommerce shipping method costs based on custom currency rate.
     *
     * @param array $rates Shipping rates.
     * @param array $package Shipping package data.
     * @return array Modified rates with converted cost.
     */
    /**
     * Convert shipping costs based on selected currency.
     */
    public function eccw_convert_shipping_costs( $rates, $package ) {

        if ( $this->ecsw_should_skip_currency_conversion() ) {
            return $rates;
        }

        $currency_rate = $this->currency_server->ecsw_get_currency_rate();
        ...
        foreach ( $rates as $rate_key => $rate ) {
            if ( isset( $rate->cost ) && $rate->cost > 0 ) {
                $rates[ $rate_key ]->cost = floatval( $rate->cost ) * $currency_rate;
            }

            if ( isset( $rate->taxes ) && is_array( $rate->taxes ) && ! empty( $rate->taxes ) ) {
                foreach ( $rate->taxes as $tax_id => $tax_cost ) {
                    $rates[ $rate_key ]->taxes[ $tax_id ] = floatval( $tax_cost ) * $currency_rate;
                }
            }
        }
    ...
    /**
     * Clear WooCommerce cached shipping rates when currency changes.
     */
    public function eccw_clear_cached_shipping_rates( $cart ) {
        if ( isset( $_COOKIE['user_preferred_currency'] ) ) {
            $current_currency = sanitize_text_field( wp_unslash( $_COOKIE['user_preferred_currency'] ) );
            $session_currency = WC()->session->get( 'ecsw_shipping_currency' );

            if ( $current_currency !== $session_currency ) {
                WC()->session->set( 'ecsw_shipping_currency', $current_currency );
