...
function weiboo_modal_pop() {
	...
	// Check if $weiboo_option exists and is an array
	if ( ! isset($weiboo_option) || ! is_array($weiboo_option) ) {
		return;
	}
	
	// Get values with proper checks and fallbacks
	$wsm_title = isset($weiboo_option['wsm_title']) ? $weiboo_option['wsm_title'] : '';
	$wsm_subtitle = isset($weiboo_option['wsm_subtitle']) ? $weiboo_option['wsm_subtitle'] : '';
	$wsm_shortcode = isset($weiboo_option['wsm_shortcode']) ? $weiboo_option['wsm_shortcode'] : '';
	$wsm_notice = isset($weiboo_option['wsm_info_notice']) ? $weiboo_option['wsm_info_notice'] : '';
	
	// Handle background image with proper checks
	$wsm_bg = '';
	$banner = '';
	if ( isset($weiboo_option['wsm_bg']) && is_array($weiboo_option['wsm_bg']) && isset($weiboo_option['wsm_bg']['id']) ) {
		$wsm_bg = $weiboo_option['wsm_bg']['id'];
		$banner_src = wp_get_attachment_image_src($wsm_bg, 'large');
		if ( $banner_src && isset($banner_src[0]) ) {
			$banner = $banner_src[0];
		}
	}
	
	$wsm_enabled = isset($weiboo_option['enable_subscription_modal']) ? $weiboo_option['enable_subscription_modal'] : false;
